...
from flask import Blueprint, render_template, request, flash, redirect, url_for
from flask_mail import Message
from app import mail  # Import your mail instance correctly
...
def schedule():
    if request.method == 'POST':
        ...
        msg = Message(
            subject="Offline Interview Invitation",
            sender=from_email,
            recipients=[to_email],
            body=f"Interview Date: {interview_date}\n\nMessage: {message_content}"
        )
        mail.send(msg)
...
# from flask import render_template, current_app
# import os
# import pdfplumber  # Ensure pdfplumber is installed: pip install pdfplumber




@main.route('/view_candidates/<int:job_id>', endpoint='view_candidates_for_job')
def view_candidates_for_job(job_id):
    ...
    for application in applications:
        ...
        candidates.append({
            ...
            'resume_filename': user.cv_file
        })

    return render_template('view_candidates.html', job=job, candidates=candidates)
    ...
    if g.user.cv_file:
        ...
        if os.path.isfile(cv_path):
            try:
                ...
                logging.error(f"Error extracting CV text: {e}")
                flash('Unable to load your CV content.', 'warning')
        else:
            flash('CV file not found, please re-upload.', 'danger')

    return render_template('job_detail.html', job=job, cv_text=cv_text)
...
#             msg = Message(
#                 subject="Offline Interview Invitation",
#                 sender=from_email,  # Use app.config['MAIL_USERNAME'] if sender is fixed
#                 recipients=[to_email],
#                 body=f"Interview Date: {interview_date}\n\nMessage: {message_content}"
#             )
#             mail.send(msg)
#             flash("Email Sent Successfully!", "success")
#         except Exception as e:
#             print(f"Error sending email: {e}")  # Logs the error to the terminal
